<template>
    ...
    <div class="w-full flex justify-start items-center px-6 py-4 gap-3 bg-white rounded-md mb-3 shadow-md">
        ...
        <!-- breadcrumb link -->
        <RouterLink :to="{name:'category'}" class="decoration-0 text-black text-sm font-medium">
            Category
        </RouterLink>
        <!-- / breadcrumb link -->
    ...
    <div class="w-full min-h-[calc(100vh-370px)] max-h-[calc(100vh-370px)] overflow-y-auto pe-3 py-3" v-if="tableData.length > 0 && !listLoading">
        ...
        <div class="w-full grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-5">
            ...
            <template v-for="(each, index) in tableData" :key="index">
                ...
                <!-- card -->
                <div class="w-full bg-white rounded-md shadow-lg group duration-500 hover:shadow-2xl shadow-gray-400 overflow-hidden">

                    <!-- card title -->
                    <div class="text-sm px-7 py-4 font-medium min-h-[150px] max-h-[150px] flex justify-center items-center flex-col">
                        <div class="mb-3">
                            {{each.title}}
                        </div>
                        <div class="flex justify-center items-center gap-5">
                            <button type="button" class="outline-0 border-0 bg-transparent text-gray-600 cursor-pointer" @click="openManageModal(each.id)">
                                Edit
                            </button>
                            <button type="button" class="outline-0 border-0 bg-transparent text-red-600 cursor-pointer" @click="openDeleteModal(each.id)">
                                Delete
                            </button>
                        </div>
                    </div>
    ...
    methods: {
        ...
        async createApi() {
            try {
                ...
                await axios.post(apiRoutes.createCategory,formData,{headers:{'Content-Type':'multipart/form-data'}});
        ...
        async updateApi() {
            try {
                ...
                formData.append('title', this.formData.title);
                await axios.post(apiRoutes.updateCategory+`/${this.formData.id}?_method=PUT`,formData,{headers:{'Content-Type': 'multipart/form-data'}});
                this.closeManageModal();
                await this.listApi();
            } catch(e) {
                this.error = e.response.data.errors;
        ...
        async listApi() {
            try {
                ...
                const response = await axios.get(apiRoutes.listCategory, {params: this.params}, {headers: apiServices.headerContent});
        ...
        async showApi(data) {
            try {
                ...
                const response = await axios.get(apiRoutes.showCategory+`/${data}`, {headers: apiServices.headerContent});
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.deleteCategory+`/${this.formData.id}`, {headers: apiServices.headerContent});
        ...
        async clearApi() {
            try {
                this.deleteLoading = true;
                await axios.delete(apiRoutes.clearCategory, {headers: apiServices.headerContent});
                await this.listApi();
            } finally {
                this.deleteLoading = false;
            }
